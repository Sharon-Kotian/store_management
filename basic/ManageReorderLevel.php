<?php
	...
	if(isset($_POST['submit'])){
		...
		$rm_reorder_level=$_POST['rm_reorder_level'];

		$rm_name=mysqli_real_escape_string($connection,$rm_name);
		$rm_reorder_level=mysqli_real_escape_string($connection,$rm_reorder_level);

		$insert_query=mysqli_query($connection, "UPDATE raw_materials SET rm_reorder_level='$rm_reorder_level' WHERE rm_name='$rm_name'");
		if($insert_query){
			echo '<script type="text/javascript">alert("Reorder Level Submitted Successfully.");
			window.location.replace("ManageReorderLevel1.php")</script>';
		}
		else{
	...
	<div id="formContent"><br><br>
		...
		<table style="width:60%" id="formContent" align="center">
			...
			<?php
				...
				$sql=mysqli_query($connection, "SELECT rm_reorder_level FROM raw_materials WHERE rm_name='$rm_name'");
					while($data=mysqli_fetch_array($sql)){

			?>		
				<td align="right" class="form-label">Select Raw Material: </td>
				<td align="center"><input type="text" id="rm_name" class="form-control" name="rm_name" value="<?php echo $rm_name; ?>" style="width: 300px; height: 40px;" readonly></td>
			</tr>
			<tr align="center">
				<td align="right" style="width: 300px; height: 20px;" class="form-label"> <br><br>Existing Reorder level: </td>
				<td align="center"><br><br><input type="text" id="ext_reorder_level" class="form-control" name="ext_reorder_level" style="width: 300px; height: 40px;" value="<?php echo $data['rm_reorder_level']; ?>" readonly></td>
				<?php 	} ?>
			</tr>
			<tr align="center">
				<td align="right" style="width: 300px; height: 20px;" class="form-label"> <br><br>Reorder level: </td>
				<td align="center"><br><br><input type="text" id="rm_reorder_level" class="form-control" name="rm_reorder_level" style="width: 300px; height: 40px;" required></td>
			</tr>
		</table><br><br>
