	...
	<form name="frm" method="post">
		<table style="width:50%" id="tableContent" align="center">
			<tbody id="table">
				...
				<td align="center"><br><br><br><br><select multiple name="rm_name[]" id="rm_name" style="width: 300px; height: 45px;" class="form-select select2-hidden-accessible" data-select2-id="select2-data-fpd_material_name" tabindex="-1" aria-hidden="true">
				<?php 
			...
			<?php echo "<input type='submit' id='clickme' value='Display' class='btn btn-primary btn-lg' style='width: 150px; height: 45px;' name='delete'>"; 
				...
				if(isset($_POST['delete'])){
					...
					while ($row=mysqli_fetch_array($final_record)){
						...
						<td align='center'>
							<select name="caret_id_update[]" id="caret_id_update" style="width: 200px; height: 45px;" class="form-select">
								...
								<?php 
									...
									while($data=mysqli_fetch_array($records)){
										...
										$rm_caret_id = $row['rm_caret_id'];		//This is the original caret ID present in DB (raw_materials table)
					...
					echo '</tbody>';
                    echo '</table>';
					echo '<input type="submit" value="Update Caret" class="btn btn-primary btn-lg" style="width: 150px; height: 45px;" name="update_caret">';
				}
