...
<head>
	...
	<title>Job Work Receivable</title>
	<meta charset="utf-8">
...
<body class="bg-light text-dark">
	...
	<h1 class="display-3" align="center">Job Work Receivable</h1>
</header>
...
<div class="wrapper fadeInDown"> <div id="formContent">
		<p align="center">
			<br><br>
			<a class="btn btn-primary btn-lg" href="job_work_receivable.php?source=Date" ONCLICK="ShowRowMaterial()" style="width: 250px; height: 50px;">Date Wise</a>
			<a class="btn btn-primary btn-lg" href="job_work_receivable.php?source=Material" ONCLICK="ShowProduct()" style="width: 250px; height: 50px;">Material Wise</a>
		<a class="btn btn-primary btn-lg" href="job_work_receivable.php?source=Worker" ONCLICK="ShowRowMaterial()" style="width: 250px; height: 50px;">Job Worker Wise</a>
			</p>
			...
			<?php
				...
				}	switch ($source)
				{
					case 'Material':
						...
						$query = "SELECT DISTINCT jw_good_name FROM ext_job_work where jw_actual_date IS NULL";
						...
						if(isset($_POST['igs_name_submit']))
						{
							...
							//echo $igs_name;
							$query = "select DISTINCT * from ext_job_work where jw_good_name='{$igs_name}' AND jw_actual_date IS NULL";
						...
						<?php	
							...
							{
								//$selectquery = mysqli_query($connection,"select jw_good_name,jw_exp_qnty,jw_act_qnty,jw_submit_date,jw_actual_date,jw_tot_amt from ext_job_work where jw_good_name = '$igs_name' ORDER BY jw_id");
								...
								//echo "<td align='center'>$jw_submit_date</td>";
								//echo "<td align='center'>$jw_actual_date</td>";          
                      ...
                      case 'Date':				
						...
						if(isset($_POST['historic_job_work_submit']))
						{
							...
							$query = "SELECT * from ext_job_work where date_format(jw_submit_date, '%Y-%m-%d')>='$jw_actual_date' AND date_format(jw_submit_date, '%Y-%m-%d')<='$jw_actual_date1' and jw_actual_date IS NULL ORDER BY jw_actual_date ASC ";
							$raw_mat_query = mysqli_query($connection,$query);
						?>
					...
					case 'Worker':
						...
						$query = "SELECT DISTINCT ew_id, ew_name FROM ext_worker,ext_job_work where ext_worker.ew_id=ext_job_work.jw_worker_id and ext_job_work.jw_actual_date='0000-00-00' and not ext_worker.ew_status = 'Deleted'";
						$raw_mat = mysqli_query($connection,$query);
						...
						while ($row = mysqli_fetch_assoc($raw_mat)) 
						{
							$ew_id = $row['ew_id'];
							$ew_name = $row['ew_name'];
							echo "<option value=$ew_id>$ew_name</option>";
						}
						echo "</select>";
						echo '<br><br>';
						...
						if(isset($_POST['ew_name_submit']))
						{
							...
							//echo $igs_name;
							$query = "select DISTINCT * from ext_job_work where jw_worker_id={$ew_name} AND jw_actual_date IS NULL";
							$raw_mat_query = mysqli_query($connection,$query);
						?>
						<table class="table table-bordered table-hover" style="width:80%">
							<thead>
						...
						<?php	
							...
							while ($row = mysqli_fetch_assoc($raw_mat_query)) 
							{
								//$selectquery = mysqli_query($connection,"select jw_good_name,jw_exp_qnty,jw_act_qnty,jw_submit_date,jw_actual_date,jw_tot_amt from ext_job_work where jw_actual_date IS NULL AND jw_worker_id = $ew_name ORDER BY jw_id");
								//while($rows = mysqli_fetch_assoc($selectquery)){
								$jw_good_name = $row['jw_good_name'];
								$jw_exp_qnty= $row['jw_exp_qnty'];
								$jw_act_qnty = $row['jw_act_qnty'];
