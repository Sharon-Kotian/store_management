				...
				else
				{
					$source = '';
						...
						if(isset($_POST['supp_name_submit']))
						{
							...
							$query = "SELECT * from raise_requisition where date_format(rr_req_date, '%Y-%m-%d')>='$rr_req_date' AND date_format(rr_req_date, '%Y-%m-%d')<='$rr_req_date1' AND rr_remaining_quantity!= 0 ORDER BY cast(rr_req_date as int) ASC ";
